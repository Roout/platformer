    ...
    if (spriteDisplay)
    {
        ...
        case BlendMode::Add:
        {
            ...
            if (texture && texture->hasPremultipliedAlpha())
            {
				// \update
				// \lib cocos2dx 4.0
				// \brief switched to cocos2d::backend::BlendFactor::ONE from the deprecated macros GL_ONE
				// \author Roout
				// \date 24.03.2020
...
// \update
// \lib cocos2dx 4.0
// \brief add type conversation to get rid of the loss data warnings 
// \author Roout
// \date 24.03.2020
void CCSlot::_updateColor()
{
    _renderDisplay->setOpacity(static_cast<uint8_t>(_colorTransform.alphaMultiplier * 255.0f));

    static cocos2d::Color3B helpColor;
    helpColor.r = static_cast<uint8_t>(_colorTransform.redMultiplier * 255.0f);
    helpColor.g = static_cast<uint8_t>(_colorTransform.greenMultiplier * 255.0f);
    helpColor.b = static_cast<uint8_t>(_colorTransform.blueMultiplier * 255.0f);
